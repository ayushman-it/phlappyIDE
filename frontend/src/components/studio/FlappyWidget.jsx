...
import { Volume2, Sparkles, MessageSquare, Minimize2, Maximize2, X } from 'lucide-react';
...
export const FlappyWidget = () => {
  ...
  if (isFlappyHidden) {
    return (
      <button
        ...
        className={`fixed bottom-8 right-6 z-40 ${theme.dropdownBg} p-2.5 rounded-full shadow-2xl border transition-all flex items-center space-x-1.5 text-xs font-bold cursor-pointer`}
    ...
    return (
      <div className="fixed bottom-8 right-6 z-40">
        <button
          ...
          className={`relative group ${theme.dropdownBg} border p-2 rounded-full shadow-2xl transition-all cursor-pointer flex items-center space-x-2 ${
        ...
        >
          ...
          {isSpeaking ? (
            ...
            <span className="text-[11px] font-bold pr-1">Phlappy AI</span>
          )}
  ...
  return (
    <div className="fixed bottom-8 right-6 z-40 max-w-[420px]">
      <div className={`${theme.dropdownBg} backdrop-blur-md rounded-2xl px-3.5 py-2.5 border shadow-2xl flex flex-col space-y-2 transition-all animate-in fade-in slide-in-from-bottom-2 duration-200`}>
        ...
        <div className="flex items-center justify-between border-b border-slate-200/40 pb-1.5">
          <div className="flex items-center space-x-1.5">
            <span className="font-extrabold text-[11px] tracking-tight flex items-center gap-1">
              Phlappy AI Teacher
          ...
          <div className="flex items-center space-x-1">
            <button
              ...
              className="p-1 opacity-60 hover:opacity-100 rounded-md transition-opacity cursor-pointer"
        ...
        <div className="flex items-center space-x-2.5 pt-0.5">
          ...
          {/* Speech Text */}
          <p className={`flex-1 text-[11px] font-semibold leading-relaxed line-clamp-3 ${theme.textMuted}`}>
            "{flappyState.speechText}"
          </p>

          {/* Audio Indicator Icon */}
          <div className={`flex-shrink-0 p-1.5 rounded-lg border ${theme.pillBg}`}>
            {isSpeaking ? (
